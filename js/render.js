...
function renderDisclaimerPreviewAndUsage(disclaimer){ const preview=document.getElementById("disclaimerPreview"), usage=document.getElementById("disclaimerUsage"); if(preview){preview.innerHTML=""; preview.appendChild(el("h4",{},["Formatted preview"])); preview.appendChild(el("div",{class:"disclaimer"},[disclaimer.DisclaimerText||""]));} if(usage){ const linked=personasUsingDisclaimer(disclaimer.DisclaimerID); usage.innerHTML=""; usage.appendChild(el("h4",{},["Linked personas"])); usage.appendChild(linked.length?el("ul",{},linked.map(p=>el("li",{},[`${p.PersonaName||p.PersonaID} (${p.PersonaID})`]))):el("p",{class:"editor-warning"},["No personas link to this disclaimer."])); if(linked.length>1) usage.appendChild(el("p",{class:"editor-warning"},["Warning: this disclaimer is used by multiple personas."])); } }
